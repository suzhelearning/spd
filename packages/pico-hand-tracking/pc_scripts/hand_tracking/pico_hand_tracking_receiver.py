#!/usr/bin/env python3
"""Receive PICO hand tracking frames over an ADB-forwarded TCP socket.
...
when another process owns the ADB forwarding lifecycle.
    ...
    python pc_scripts/hand_tracking/pico_hand_tracking_receiver.py --visualize

The receiver uses only the Python standard library until ``--visualize`` is
requested.  Visualization then requires matplotlib.  Coordinates are FLU:
...
# PICO HandJoint enum order.  These indices are also the order in every frame.
JOINT_NAMES = (
    "palm", "wrist",
...
class HandTrackingReceiver:
    ...
    def start(self) -> threading.Thread:
        thread = threading.Thread(target=self.run, name="pico-hand-receiver", daemon=True)
        thread.start()
        return thread
    ...
    def ensure_adb_forward(self) -> None:
        ...
        intentionally re-run after every failed socket connection so a PICO
...
def run_qt_visualizer(receiver: HandTrackingReceiver) -> None:
    ...
    app.setApplicationName("PICO Hand Tracking")
    app.setStyle("Fusion")

    window = QtWidgets.QMainWindow()
    window.setWindowTitle("PICO Hand Tracking  ·  FLU Dashboard")
    ...
    brand = QtWidgets.QLabel("PICO HAND TRACKING")
...
def run_mpl_visualizer(receiver: HandTrackingReceiver) -> None:
    ...
    plt.ion()
    figure = plt.figure("PICO Hand Tracking · FLU", figsize=(10, 8), facecolor="#07111f")
    ...
    while not receiver.stop_event.is_set() and plt.fignum_exists(figure.number):
        ...
        axes.set_title(
            "PICO Hand Tracking  ·  FLU rear view",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Receive PICO FLU hand tracking over ADB forward TCP")
    parser.add_argument("--host", default="127.0.0.1", help="ADB-forwarded host (default: 127.0.0.1)")
    parser.add_argument("--port", type=int, default=10002, help="must match PicoHandTrackingAdbStreamer.tcpPort")
    parser.add_argument("--device-port", type=int, help="PICO device TCP port; defaults to --port")
