...
constexpr double kPicoInputTimeoutSeconds = 0.050;
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (argument == "--help") {
      std::cout << "Usage: tianji_qp_ik_viewer [--config FILE] [--model FILE] "
                   "[--solver qpoases] [--headless] [--duration SECONDS] "
      ...
      if (classification.action == PicoTeleopAction::kApply ||
        ...
        if (target_accepted) {
          ...
          if (reset_epoch) {
            ...
            // MuJoCo feedback pose here teleported the reference whenever the
            // model reference and simulated feedback had separated.
            plot_reset_requested = true;
            ++pico_reset_applies;
          }
...
int runPicoHeadless(const Options& options,
  ...
  const auto finish = std::chrono::steady_clock::now() +
                      std::chrono::duration<double>(options.duration_seconds);
  while (running.load(std::memory_order_acquire) &&
         std::chrono::steady_clock::now() < finish) {
    drainJointPlotQueue(joint_plot_queue, plot_observation);
...
int run(int argc, char** argv) {
  ...
  int result = 0;
  try {
    if (options.headless && options.pico_teleop) {
