...
TEST(Config, LoadsPicoTeleopProfile) {
  const auto path = std::filesystem::path(TIANJI_PROJECT_SOURCE_DIR) /
                    "config" / "qp_ik_pico_teleop.yaml";
  const QpIkConfig config = loadConfig(path.string());
  EXPECT_DOUBLE_EQ(config.arm_angle.continuity_weight_scale, 30.0);
  ...
  EXPECT_TRUE(config.cartesian_otg.translation_tracking_enabled);
  EXPECT_DOUBLE_EQ(config.cartesian_otg.translation_tracking_gain, 30.0);
  EXPECT_DOUBLE_EQ(
      config.cartesian_otg.translation_stationary_velocity_threshold, 1.0e-4);
  EXPECT_TRUE(config.cartesian_otg.translation_prediction_enabled);
  EXPECT_DOUBLE_EQ(
      config.cartesian_otg.translation_prediction_horizon_seconds, 0.015);
  EXPECT_TRUE(config.cartesian_otg.stationary_hold_enabled);
  EXPECT_DOUBLE_EQ(config.cartesian_otg.stationary_hold_dwell_seconds, 0.15);
  EXPECT_DOUBLE_EQ(
      config.cartesian_otg.translation_hold_enter_velocity_m_s, 0.06);
  EXPECT_DOUBLE_EQ(
      config.cartesian_otg.translation_hold_exit_velocity_m_s, 0.12);
  EXPECT_DOUBLE_EQ(
      config.cartesian_otg.translation_hold_exit_position_error_m, 0.020);
  EXPECT_DOUBLE_EQ(
      config.cartesian_otg.orientation_hold_enter_velocity_rad_s, 0.12);
  EXPECT_DOUBLE_EQ(
      config.cartesian_otg.orientation_hold_exit_velocity_rad_s, 0.25);
  EXPECT_DOUBLE_EQ(config.cartesian_otg.orientation_hold_exit_error_rad,
                   0.035);
  EXPECT_DOUBLE_EQ(config.cartesian_otg.translation_acceleration_max, 30.0);
  EXPECT_DOUBLE_EQ(config.cartesian_otg.translation_jerk_max, 1000.0);
  EXPECT_DOUBLE_EQ(config.cartesian_otg.angular_acceleration_max, 100.0);
  EXPECT_DOUBLE_EQ(config.cartesian_otg.angular_jerk_max, 3000.0);
  EXPECT_TRUE(config.cartesian_otg_control_profiles.enabled);
  const CartesianOtgConfig velocity_otg =
      cartesianOtgConfigForControlLevel(config, ControlLevel::kVelocity);
  EXPECT_DOUBLE_EQ(velocity_otg.translation_velocity_max, 3.0);
  EXPECT_DOUBLE_EQ(velocity_otg.translation_acceleration_max, 12.0);
  EXPECT_DOUBLE_EQ(velocity_otg.translation_jerk_max, 120.0);
  EXPECT_DOUBLE_EQ(velocity_otg.angular_velocity_max, 12.0);
  EXPECT_DOUBLE_EQ(velocity_otg.angular_acceleration_max, 40.0);
  EXPECT_DOUBLE_EQ(velocity_otg.angular_jerk_max, 400.0);
  const CartesianOtgConfig acceleration_otg =
      cartesianOtgConfigForControlLevel(config, ControlLevel::kAcceleration);
  EXPECT_DOUBLE_EQ(acceleration_otg.translation_velocity_max, 3.0);
  EXPECT_DOUBLE_EQ(acceleration_otg.translation_acceleration_max, 30.0);
  EXPECT_DOUBLE_EQ(acceleration_otg.translation_jerk_max, 1000.0);
  EXPECT_DOUBLE_EQ(acceleration_otg.angular_velocity_max, 12.0);
  EXPECT_DOUBLE_EQ(acceleration_otg.angular_acceleration_max, 100.0);
  EXPECT_DOUBLE_EQ(acceleration_otg.angular_jerk_max, 3000.0);
  EXPECT_DOUBLE_EQ(config.cartesian_servo.target_timeout_seconds, 0.050);
  EXPECT_DOUBLE_EQ(config.cartesian_servo.max_linear_velocity, 3.0);
  EXPECT_DOUBLE_EQ(config.cartesian_servo.max_angular_velocity, 12.0);
  EXPECT_TRUE(config.cartesian_servo.adaptive_gain_enabled);
  EXPECT_EQ(config.qpoases.max_working_set_recalculations, 100);
  EXPECT_DOUBLE_EQ(config.qpoases.cpu_time_limit_seconds, 0.001);
  ...
  EXPECT_DOUBLE_EQ(config.acceleration_qp.arm_angle_weight, 10.0);
  EXPECT_FALSE(config.arm_angle.nullspace_only);
  EXPECT_DOUBLE_EQ(config.acceleration_qp.posture_weight, 0.02);
  EXPECT_DOUBLE_EQ(config.acceleration_qp.posture_kp, 12.0);
  EXPECT_DOUBLE_EQ(config.acceleration_qp.posture_kd, 7.0);
  EXPECT_EQ(config.iterative_dls.max_iterations, 20);
  EXPECT_FALSE(config.iterative_dls.posture_reference_enabled);
  EXPECT_DOUBLE_EQ(config.iterative_dls.nominal_posture_gain, 0.05);
  EXPECT_DOUBLE_EQ(config.iterative_dls.arm_angle_gain, 0.025);
  EXPECT_DOUBLE_EQ(config.iterative_dls.maximum_goal_step_rad, 0.02);
  EXPECT_DOUBLE_EQ(config.iterative_dls.posture_reference_margin_rad, 0.05);
  EXPECT_DOUBLE_EQ(config.iterative_dls.minimum_merit_improvement, 1.0e-10);
  EXPECT_EQ(config.spark_upper_qpoases.calibration_frames, 30);
  EXPECT_EQ(config.spark_upper_qpoases.stage1_max_iterations, 10);
  EXPECT_EQ(config.spark_upper_qpoases.stage2_max_iterations, 10);
  EXPECT_DOUBLE_EQ(config.spark_upper_qpoases.stage2_palm_position_weight,
                   10.0);
  EXPECT_DOUBLE_EQ(config.spark_upper_qpoases.posture_weight, 2.0e-2);
  EXPECT_DOUBLE_EQ(config.spark_upper_qpoases.posture_position_gain, 4.0);
  EXPECT_DOUBLE_EQ(config.spark_upper_qpoases.joint_reference_weight, 3.0e4);
  EXPECT_DOUBLE_EQ(
      config.spark_upper_qpoases.joint_reference_attack_seconds, 0.8);
  EXPECT_DOUBLE_EQ(
      config.spark_upper_qpoases.joint_reference_release_seconds, 0.4);
  EXPECT_DOUBLE_EQ(
      config.spark_upper_qpoases.joint_reference_smoothness_weight, 3.0e3);
  EXPECT_DOUBLE_EQ(
      config.spark_headroom_feedforward_velocity_qp
  ...
  EXPECT_DOUBLE_EQ(
      ...
      1.0e5);
  EXPECT_TRUE(config.spark_upper_qpoases.enforce_hard_jerk_bounds);
  EXPECT_DOUBLE_EQ(config.dls_posture_ruckig.velocity_scale, 0.6);
  EXPECT_FALSE(config.dls_posture_ruckig.enabled);
  EXPECT_TRUE(config.dls_posture_ruckig.max_velocity_rad_s.isApprox(
      (Vec7() << 0.8, 0.8, 1.0, 1.0, 1.2, 1.2, 1.2).finished()));
  EXPECT_TRUE(config.dls_posture_ruckig.max_acceleration_rad_s2.isApprox(
      (Vec7() << 7.854, 7.854, 15.708, 15.708, 15.708, 15.708,
       15.708).finished()));
  EXPECT_TRUE(config.dls_posture_ruckig.max_jerk_rad_s3.isApprox(
  ...
  EXPECT_TRUE(config.joint_acceleration_limits.braking_acceleration_rad_s2.isApprox(
      (Vec7() << 45.0, 45.0, 45.0, 67.5, 67.5, 67.5, 67.5).finished()));
  EXPECT_TRUE(config.joint_acceleration_limits.max_jerk_rad_s3.isApprox(
  ...
  EXPECT_TRUE(config.joint_limits.braking_acceleration_rad_s2.isApprox(
      (Vec7() << 45.0, 45.0, 45.0, 67.5, 67.5, 67.5, 67.5).finished()));
  EXPECT_FALSE(config.joint_limits.hard_jerk_enabled);
  EXPECT_TRUE(config.joint_limits.max_jerk_rad_s3.isApprox(
      (Vec7() << 1000.0, 1000.0, 1000.0, 1500.0, 1500.0, 1500.0,
       1500.0).finished()));
  EXPECT_DOUBLE_EQ(config.hierarchical_qp.arm_angle_weight, 3.0);
  EXPECT_DOUBLE_EQ(config.hierarchical_qp.wrist_posture_weight_scale, 100.0);
  EXPECT_DOUBLE_EQ(config.hierarchical_qp.jerk_weight, 2.0e-2);
  EXPECT_TRUE(config.hierarchical_qp.task_scaling_enabled);
  EXPECT_DOUBLE_EQ(config.hierarchical_qp.task_scaling_min_position, 0.75);
  EXPECT_DOUBLE_EQ(config.hierarchical_qp.task_scaling_min_orientation, 0.75);
  EXPECT_DOUBLE_EQ(config.hierarchical_qp.task_scaling_weight_position,
                   8000.0);
  EXPECT_DOUBLE_EQ(config.hierarchical_qp.task_scaling_weight_orientation,
                   3000.0);
  EXPECT_DOUBLE_EQ(config.joint_acceleration_limits.velocity_scale, 1.0);
  EXPECT_DOUBLE_EQ(config.joint_limits.velocity_scale, 1.0);
  EXPECT_DOUBLE_EQ(config.pico_teleop.max_position_jump_m, 0.15);
  EXPECT_DOUBLE_EQ(config.pico_teleop.max_orientation_jump_rad, 0.60);
  EXPECT_DOUBLE_EQ(config.arm_angle.reference_rate_limit_rad_s, 2.0);
  EXPECT_DOUBLE_EQ(config.arm_angle.continuity_kp_velocity, 8.0);
  EXPECT_DOUBLE_EQ(config.arm_angle.continuity_max_velocity_rad_s, 2.5);
  EXPECT_DOUBLE_EQ(config.arm_angle.reference_projection_hold_enter, 0.05);
  EXPECT_DOUBLE_EQ(config.arm_angle.reference_projection_hold_exit, 0.10);
  EXPECT_DOUBLE_EQ(config.arm_angle.branch_lock_radius_m, 0.040);
  EXPECT_DOUBLE_EQ(config.arm_angle.full_weight_radius_m, 0.050);
  EXPECT_FALSE(config.arm_angle.reference_governor_enabled);
  EXPECT_DOUBLE_EQ(config.arm_angle.reference_governor_enter_error_rad, 1.50);
  EXPECT_DOUBLE_EQ(config.arm_angle.reference_governor_exit_error_rad, 1.30);
  EXPECT_DOUBLE_EQ(config.arm_angle.reference_governor_tracking_error_rad,
                   1.20);
  EXPECT_FALSE(config.arm_angle.tracking_envelope_enabled);
  EXPECT_DOUBLE_EQ(config.arm_angle.tracking_envelope_max_error_rad, 1.50);
  EXPECT_DOUBLE_EQ(config.arm_angle.tracking_envelope_gain, 8.0);
  EXPECT_DOUBLE_EQ(config.upper_arm_outward.minimum_outward_distance_m, 0.0);
  EXPECT_DOUBLE_EQ(config.upper_arm_outward.velocity_gain, 8.0);
  EXPECT_DOUBLE_EQ(config.upper_arm_outward.acceleration_kp, 100.0);
  EXPECT_DOUBLE_EQ(config.upper_arm_outward.acceleration_kd, 20.0);
