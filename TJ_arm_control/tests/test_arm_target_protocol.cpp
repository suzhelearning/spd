...
ArmTargetFrame validFrame() {
  ...
  frame.hold_reason = ArmTargetHoldReason::kNone;
...
TEST(ArmTargetProtocol, EncodesAndDecodes272ByteLittleEndianPacket) {
  ...
  std::array<std::uint8_t, kArmTargetPacketV1Size> bytes{};
  ...
  EXPECT_EQ(decoded.frame->hold_reason, expected.hold_reason);
  for (std::size_t index = 0U; index < 7U; ++index) {
...
TEST(ArmTargetProtocol, MatchesSharedPythonFixtureBytes) {
  const std::string fixture_path =
      ...
      "/../PICO_tracker/src/spd_vr/test/fixtures/arm_target_v1.hex";
  std::ifstream fixture(fixture_path);
  ASSERT_TRUE(fixture.good()) << fixture_path;
  std::string hex;
  fixture >> hex;
  ASSERT_EQ(hex.size(), kArmTargetPacketV1Size * 2U);
  std::array<std::uint8_t, kArmTargetPacketV1Size> expected{};
  for (std::size_t index = 0U; index < expected.size(); ++index) {
    expected[index] = static_cast<std::uint8_t>(
        std::stoul(hex.substr(index * 2U, 2U), nullptr, 16));
  }
  std::array<std::uint8_t, kArmTargetPacketV1Size> actual{};
...
TEST(ArmTargetProtocol, RejectsCorruptCrcAndNonFiniteValues) {
  auto bytes = std::array<std::uint8_t, kArmTargetPacketV1Size>{};
...
TEST(ArmTargetProtocol, EncodesAndDecodesPartialHoldReason) {
  ...
  frame.hold_reason = ArmTargetHoldReason::kInputStale;
  std::array<std::uint8_t, kArmTargetPacketV1Size> bytes{};
  ...
  EXPECT_EQ(decoded.frame->hold_reason, ArmTargetHoldReason::kInputStale);
}
