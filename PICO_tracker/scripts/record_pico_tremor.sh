...
output_root=""
dry_run=false
caller_dir="${PICO_TREMOR_CALLER_DIR:-$PWD}"
original_args=("$@")
...
resolve_pixi_bin() {
  if [[ -n "${PIXI_BIN:-}" && -x "${PIXI_BIN}" ]]; then
    ...
    echo "找不到 pixi；请安装 pixi 或设置 PIXI_BIN" >&2
    return 1
  fi
}

if [[ "${PICO_TREMOR_CLEAN_ENV:-0}" != "1" ]]; then
  ...
  exec env \
    ...
    -u CYCLONEDDS_URI \
    PICO_TREMOR_CLEAN_ENV=1 \
    PICO_TREMOR_CALLER_DIR="$caller_dir" \
    PIXI_PROJECT_ROOT="$repo_root" \
