...
workspace is managed with Pixi and includes PICO streaming, dual IMU900 input,
...
repository's Pixi shell. The Tianji tmux launcher enters Pixi automatically.

For local PICO and exoskeleton testing, keep the following values identical in
every terminal after entering `pixi shell`:
...
```bash
cd /home/zj/current_robotics/PICO_tracker
./scripts/start_tianji_pico_teleop.sh
```
...
Inspect input and bridge diagnostics in any repository Pixi terminal:
...
```bash
cd /home/zj/current_robotics/TJ_arm/TJ_arm_control_DLS_IK
./build_dls/tianji_qp_ik_viewer \
...
- [pixi](https://prefix.dev) (installs ROS 2 Humble and workspace dependencies)
- Linux x86_64
- `adb` on `PATH`
...
```bash
git clone git@github.com:zhoujie19961123/PICO_tracker.git
cd PICO_tracker
pixi install
pixi shell
...
that side's TCP orientation. In the project `pixi shell`, use:
...
driver online and run the selected side from the Pixi shell:
