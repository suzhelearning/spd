...
This is a **pixi-managed ROS2 Humble workspace** (via the `robostack-staging` conda channel). There is no system-wide ROS install — every build/test/run command must go through pixi.

```bash
pixi install            # first time only; downloads Humble
pixi shell              # enter env (all commands below assume this shell)
```

After a build, always `source install/setup.bash` before `ros2 run` / `ros2 launch`.

## Common commands

```bash
# Build / test (pixi tasks wrap colcon and pin the three packages)
pixi run build                                 # colcon build --symlink-install --packages-select ...
pixi run test                                  # colcon test --event-handlers console_direct+
...
# Run (three terminals; each must be in `pixi shell` with `install/setup.bash` sourced)
...
- Pins in `pixi.toml` are load-bearing: `setuptools <80` (colcon's ament_python build still uses the removed `setup.py develop`), `cmake <4`, and the build task's `-DPython_ROOT_DIR=$CONDA_PREFIX` (CMake's FindPython doesn't auto-detect conda envs). Don't loosen them without testing a clean rebuild.
