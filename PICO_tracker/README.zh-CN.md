...
PICO 4U 数据采集与脚部姿态融合流水线。本 ROS 2 Humble 工作空间使用 Pixi 管理，
包含 PICO 数据流、双 IMU900 输入、鱼眼相机、Odin 里程计、数据录制和 MuJoCo 可视化。

## 快速启动：PICO 掌心约束骨架

快速脚本默认使用 `ROS_DOMAIN_ID=120`、`ROS_LOCALHOST_ONLY=1`，并关闭
ROS 2 CLI daemon。标定和 PICO 单独验证在仓库的 Pixi shell 中运行；天机 tmux
启动脚本会自动进入 Pixi 环境。
...
- [pixi](https://prefix.dev)（安装 ROS 2 Humble 和工作空间依赖）
- Linux x86_64
- `PATH` 中可用的 `adb`
...
先启动 PICO bridge，再在当前仓库进入 Pixi shell。脚本默认使用 domain 120；只在本机
...
`pico_*_arm_geometry_quick_v3` 候选原子激活到 `~/.config/pico_tracker/`。标定器的交互动作
...
骨长标定。进入本项目的 `pixi shell`，只重标所选侧 TCP 姿态：
...
保持 PICO 驱动在线，并在 Pixi shell 中直接选择对应侧的骨长标定：
...
终端输出限频警告。MuJoCo 由 `pixi install` 安装。
