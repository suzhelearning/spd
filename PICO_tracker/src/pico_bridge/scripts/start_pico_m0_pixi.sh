...
if [[ -f "$source_repo_root/pixi.toml" ]]; then
  repo_root="$source_repo_root"
elif [[ -n "${PIXI_PROJECT_ROOT:-}" && -f "$PIXI_PROJECT_ROOT/pixi.toml" ]]; then
  repo_root="$(cd "$PIXI_PROJECT_ROOT" && pwd -P)"
else
  echo "cannot locate the PICO repository; run this command from its Pixi shell" >&2
  exit 2
fi
...
usage() {
  cat >&2 <<'EOF'
usage: start_pico_m0_pixi.sh [options]
  --domain ID
