...
public:
        ...
        pico_world_x_offset_m_))
  {
    ...
    // A 键事件：driver 在收到 PICO 0x09 帧时发布 /pico/record_flag。
    // 每收到一次翻转上报状态（与 marvin_bridge 直连转发器语义一致），
    // viewer 侧对该位做上升沿检测切换 teleop 使能。
    record_flag_subscription_ = create_subscription<std_msgs::msg::Bool>(
      ...
      [this](const std_msgs::msg::Bool::SharedPtr message) {
        (void)message;
        record_button_state_ = !record_button_state_;
