...
```text
.pixi/envs/default/bin/python -m py_compile ../wuji-retargeting/wuji_retargeting/robot.py ../wuji-retargeting/wuji_retargeting/opt/base.py src/spd_vr/spd_vr/retarget_pair.py src/spd_vr/spd_vr/runtime.py && .pixi/envs/default/bin/python -c "from spd_vr.retarget_pair import WujiRetargetPair; from spd_vr.runtime import run_runtime; print('imports ok')"
```
...
Review fixes: manifest joints are read before actuator mapping; manifest self-hash is verified canonically; runtime uses `from_manifest`; malformed hand arrays/scales are replaced with validated inactive-side data rather than bypassing input validation; invalid-side regression is covered while the valid side continues.
