...
manifest/hash verification remains fail-closed; the explicit synthetic fixture
is not authoritative artifact evidence.

## Post-final fix evidence

The IK controller now declares and owns `STATUS_IK_KEY` through the shared
`ZenohNode`, publishing `ready`, `running`, `paused`, `tick_count`, and
`sequence` on connect, control transitions, and ticks. Viewer source latency is
always `unknown` without explicit clock synchronization; bridge latency uses
the tracking frame's `bridge_monotonic_ns` and the PC mailbox arrival timestamp.
...
-- Docs: https://docs.pytest.org/en/stable/how-to/capture-warnings.html
42 passed, 14 warnings in 0.71s
```
...
Shutdown status publication is normalized before deduplication: accepted
`SHUTDOWN`, a subsequent tick, and final cleanup produce at most one final
status. Cleanup always attempts `node.close()` even when final status
publication fails, while preserving the original exit path.
...
test/test_arm_ik.py: 14 warnings
  Warning: "polish" is deprecated. Please use "polishing" instead.
...
-- Docs: https://docs.pytest.org/en/stable/how-to/capture-warnings.html
44 passed, 18 warnings in 0.72s
```
